...
function xmldb_quiz_upgrade($oldversion) {
    ...
    if ($oldversion < 2025062600) {
        ...
        upgrade_mod_savepoint(true, 2025062600, 'quiz');
    }

    if ($oldversion < 2026042002) {

        $table = new xmldb_table('quiz');
        $field = new xmldb_field('timernotifydegraded', XMLDB_TYPE_INTEGER, '1', null, null, null, null, 'allowofflineattempts');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        if (get_config('quiz', 'timernotifydegraded') === false) {
            set_config('timernotifydegraded', 1, 'quiz');
        }
        if (get_config('quiz', 'timernotifysuccess') === false) {
            set_config('timernotifysuccess', 1, 'quiz');
        }

        upgrade_mod_savepoint(true, 2026042002, 'quiz');
    }
